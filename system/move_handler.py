...
from workflow_models.action import LocationAction, MoveAction
from workflow_models.route import Route
...
class MoveActionCollectionReservationRequest:
    ...
    async def reserve_route(self, reservation_manager: IReservationManager) -> MoveAction:
        ...
        done, pending = await asyncio.wait([action.reservation.completed.wait() for action in self._requested_move_actions], return_when=asyncio.FIRST_COMPLETED)
