...
class HamiltonResponse:
    ...
    def raise_first_exception(self):
        ...
        if isHamiltonStepError:
            raise HamiltonStepError(
                'Hamilton step did not execute correctly; no error code given. ( response: ' + self.raw + ' )')
        ...
        isFailedStatusNoReportedErrorCode = self.status == HamiltonResponseStatus.FAILED and len(
            reportedErrorCodes) == 0
        if isFailedStatusNoReportedErrorCode:
            raise HamiltonReturnParseError(
                'Hamilton step did not execute correctly; no error code found. ( response: ' + self.raw + ' )')
        ...
        if self.status == HamiltonResponseStatus.SUCCESS:
            raise HamiltonReturnParseError(
                'Inconsistency: Venus returns SUCCESS while error code {firstErrorCode} found! ( response: ' + self.raw + ' )')
...
class HamiltonInterface:
    ...
    def send_command(self, template=None, block_until_sent=False, *args, **cmd_dict):  # returns unique id of command
        ...
        if 'id' not in send_cmd_dict:
            self.log_and_raise(
                ValueError("Command dicts sent from HamiltonInterface must have a unique id with key 'id'"))
    ...
    def wait_on_response(self, id, timeout=60, raise_first_exception=False, return_data=None):
        ...
        if server_response is None:
            self.log_and_raise(HamiltonTimeoutError(
                'Timed out after ' + str(timeout) + ' sec while waiting for response id ' + str(id)))
