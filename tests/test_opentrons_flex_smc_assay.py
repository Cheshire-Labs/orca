"""Tests for the Opentrons Flex SMC assay example (examples/opentrons_flex_smc).

This assay authors real inline liquid-handler methods pushed to the driver via
PyLabRobot across an Opentrons Flex pair (flex_1 capture/detection, flex_2
elution/neutralize). It is the Hamilton SMC assay re-instrumented onto Flexes:
the chemistry is identical, only the deck model changes. The whole 96-well
plate is processed,
every liquid-handler step is an 8-channel transfer looped over all 12 columns,
reagents are deck-resident troughs, and up to four assay plates pool into the
384-well read plate (this single-plate run writes the read plate's quadrant 0).

The point of running on a Flex is tip-box transit: the SMC assay routes a fresh
tip box per pipetting action, which a Hamilton (human-loaded rail-carrier tips)
cannot do. The DDR arm drops each transit labware at one of the Flex's two
handoff slots and the Flex's internal gripper relays it to the working slot the
action names -- ``test_transit_relays_through_a_handoff_slot`` pins that path.

The full assay runs ONCE per module (`flex_run` fixture) with MethodTracker +
...
4. each Flex executes the exact inline pipetting the assay calls for --
...
from tests.test_helpers import assert_translator_carriage_pairs, named_for_template, template_of
from orca.runtime.system_runtime import SystemRuntime


# Deck-resident reagents (LEAVE_IN_PLACE) by template -> trough slot: beads +
# detection on flex_1's reagent slots, elution Buffer B + Buffer D on flex_2's.
_FLEX_1_RESIDENTS = {
    "bead_reservoir": "flex_1/B1-slot",
    "detection_reservoir": "flex_1/B2-slot",
}
_FLEX_2_RESIDENTS = {
    "buffer_b_reservoir": "flex_2/B3-slot",
    "buffer_d_reservoir": "flex_2/D3-slot",
}
_RESIDENT_SITES = {**_FLEX_1_RESIDENTS, **_FLEX_2_RESIDENTS}

# The two handoff slots per Flex (the DDR arm's taught entry points); the internal
# gripper relays labware from here to the working slots below.
_HANDOFF_SLOTS = {"D1-slot", "D2-slot"}

# Assay working slots per Flex (non-handoff sites the actions place labware at).
_FLEX_1_WORKING = {
    "flex_1/C1-slot", "flex_1/C2-slot", "flex_1/A1-slot", "flex_1/A2-slot",
    "flex_1/B3-slot", "flex_1/B1-slot", "flex_1/B2-slot",
}
_FLEX_2_WORKING = {
    "flex_2/C1-slot", "flex_2/C2-slot", "flex_2/C3-slot", "flex_2/A1-slot",
    "flex_2/A2-slot", "flex_2/B1-slot", "flex_2/B2-slot", "flex_2/B3-slot",
    "flex_2/D3-slot",
}
...
async def _run_flex() -> FlexRun:
    ...
    final = await asyncio.wait_for(runtime.wait_for_execution(submission), timeout=300.0)
    ...
    return FlexRun(method_tracker, journey_tracker, all_ops)


@pytest.fixture(scope="module")
def flex_run() -> FlexRun:
    return asyncio.run(_run_flex())


async def test_deck_sites_are_derived() -> None:
    """Both Flex deck configs derive their 11 flat slot sites; the two arm-taught
    handoff slots are ordinary sites and every reagent's resident slot exists."""
    build = await build_opentrons_flex_smc()

    system_map = build.system.system_map
    for flex_name, residents, working in (
        ("flex_1", _FLEX_1_RESIDENTS, _FLEX_1_WORKING),
        ("flex_2", _FLEX_2_RESIDENTS, _FLEX_2_WORKING),
    ):
        # Every deck site is an equal flat node (handoff-ness is derived from what
        # the arm teaches), so all slot sites are addressable.
        sites = {site.position_id for site in system_map.sites_of(flex_name)}
        assert working <= sites, f"{flex_name} missing working slots: {working - sites}"
        # The arm-taught entry (handoff) slots are ordinary deck sites too.
        assert f"{flex_name}/D1-slot" in sites
        assert f"{flex_name}/D2-slot" in sites
        for site in residents.values():
            assert site in sites, f"resident anchor {site} is not a deck site"


async def test_translators_declare_single_carriage() -> None:
    """Drift guard: the translators must come up on ``SimTranslatorDriver`` so
    the reservation layer gets the one-carriage exclusion groups. A factory
    change that silently hands them the generic arm sim re-arms the N=6
    head-on livelock without failing any unit test."""
    build = await build_opentrons_flex_smc()
...
class TestOpentronsFlexSmcAssay:

    def _thread_methods(self, run: FlexRun, prefix: str) -> list[list[str]]:
    ...
    def test_method_sequences_are_exact(self, flex_run: FlexRun) -> None:
    ...
    def test_labware_journeys(self, flex_run: FlexRun) -> None:
        """Every labware follows its expected physical journey; residents stay put.

        The assay plate must reach flex_1 (capture/detection) before flex_2
        ...
        # Assay plate: drives the whole run across BOTH Flexes and both corridors.
        (plate_1,) = paths("plate_1")
        assert plate_1[0] == "stacker_3/slot"
        assert plate_1[-1] == "waste_1/slot"
        assert_visits(plate_1, [
            "flex_1", "flex_2", "biotek_1", "biotek_2", "centrifuge",
        ...
        assert plate_devices.index("flex_1") < plate_devices.index("flex_2"), (
            f"plate_1 reached flex_2 before flex_1: {' -> '.join(plate_1)}"
        )

        (sample,) = paths("sample_plate")
        assert sample[0] == "stacker_1/slot"
        assert sample[-1] == "stacker_2/slot"
        assert_visits(sample, ["delidder", "flex_1"], "sample_plate")

        (neut,) = paths("neut_plate")
        assert neut[0] == "stacker_8/slot"
        assert neut[-1] == "waste_1/slot"
        assert_visits(neut, ["flex_2", "centrifuge"], "neut_plate")

        (final,) = paths("final_plate")
        assert final[0] == "stacker_4/slot"
        assert final[-1] == "plate_hotel/slot"
        assert_visits(final, ["flex_2", "centrifuge", "smc_pro"], "final_plate")

        # Every tip rack is delidded and reaches its consuming Flex.
        flex_1_tips = {"tips_sample", "tips_beads", "tips_detection"}
        for tip_name in _TIP_CONSUMERS:
            (path,) = paths(tip_name)
            assert_visits(path, ["delidder"], tip_name)
            expected_lh = "flex_1" if tip_name in flex_1_tips else "flex_2"
            assert_visits(path, [expected_lh], tip_name)

        # Deck residents never leave their anchored deck slots.
        for template_name, site in _RESIDENT_SITES.items():
            (path,) = paths(template_name)
            assert set(path) == {site}, f"{template_name} moved off its deck slot: {path}"

    def test_transit_relays_through_a_handoff_slot(self, flex_run: FlexRun) -> None:
        """Transit labware enters a Flex deck via a handoff slot and the internal
        gripper relays it onward -- the reason this assay is on a Flex.

        Each tip box is arm-delivered to one of the Flex's two handoff slots
        (D1/D2), then the ``flex_N/gripper`` carries it to its working slot. If
        the arm placed straight onto the working slot (no gripper), no handoff
        slot would appear in the path, so this discriminates the handoff model
        from direct placement.
        """
        run = flex_run
        journeys = run.journey_tracker.all_completed_journeys

        def path_for(prefix: str) -> list[str]:
            (path,) = [
                j for tid, j in journeys.items()
                if run.journey_tracker.thread_names.get(tid, "").startswith(prefix)
            ]
        ...
        flex_1_tips = {"tips_sample", "tips_beads", "tips_detection"}
        for tip_name in _TIP_CONSUMERS:
            path = path_for(tip_name)
            flex = "flex_1" if tip_name in flex_1_tips else "flex_2"
            # A handoff slot precedes a working slot, relayed by the internal gripper.
            handoff_idx = next(
                (i for i, loc in enumerate(path)
                 if loc.startswith(f"{flex}/") and loc.split("/", 1)[1] in _HANDOFF_SLOTS),
                None,
            )
            assert handoff_idx is not None, (
                f"{tip_name} never entered {flex} via a handoff slot: {' -> '.join(path)}"
            )
            working = _FLEX_1_WORKING if flex == "flex_1" else _FLEX_2_WORKING
            relayed = any(loc in working for loc in path[handoff_idx:])
            assert relayed, (
                f"{tip_name} was not relayed from a handoff slot to a working slot: "
                f"{' -> '.join(path)}"
            )
            assert any(loc.startswith(f"{flex}/gripper") for loc in path), (
                f"{tip_name} never rode the {flex} internal gripper: {' -> '.join(path)}"
            )

    def _transfer_blocks(self, run: FlexRun, device_name: str) -> list[tuple[tuple[str, float, str, float], int]]:
        ...
        for i in range(0, len(ops), 2):
            ...
            block = (template_of(asp.details.labware), asp.details.volumes[0], template_of(disp.details.labware), disp.details.volumes[0])
    ...
    def test_inline_operations(self, flex_run: FlexRun) -> None:
        """Each Flex executes the exact inline pipetting the assay calls for, in
        order, spanning all 12 columns (8 channels each), never a protocol string."""
        run = flex_run

        for device in ("flex_1", "flex_2"):
    ...
    def _net_volumes(self, run: FlexRun, labware: str) -> dict[str, float]:
    ...
    def test_net_volume_accounting_matches_recipe(self, flex_run: FlexRun) -> None:
