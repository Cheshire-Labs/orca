"""4-submission batch pooling E2E for the Hamilton SMC assay.
...
from tests.test_helpers import execution_outcome, named_for_template
import pytest
...
async def _run_join_existing_batch(
    ...
    final = await execution_outcome(runtime, first, timeout=timeout)
...
async def test_six_submissions_fill_two_384s() -> None:
    ...
    the pooling/overflow is capacity-driven, not hardcoded to five."""
    all_ops, receiver_count = await _run_join_existing_batch(6, timeout=2400.0)
