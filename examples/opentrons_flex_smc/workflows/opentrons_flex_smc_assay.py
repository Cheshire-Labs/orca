"""Workflow definition for the Opentrons Flex SMC (Single Molecule Counting) assay.

Models a representative bead-capture IL-6 immunoassay on two Opentrons Flex
liquid handlers. Every liquid-handler step is an 8-channel transfer looped
across all 12 columns of the 96-well plate.

This is the Hamilton SMC assay (``examples/hamilton_smc``) re-instrumented onto
Flexes. The chemistry is unchanged; what changes is the deck model. The SMC assay
routes a fresh tip box around the system map per pipetting action, which a
Hamilton cannot do (its tips are human-loaded rail-carrier racks, not arm-placed
boxes). On a Flex a tip box sits on a flat deck slot like a plate, so the arm can
transit it: the DDR arm drops it at one of the Flex's two handoff slots and the
Flex's internal gripper relays it to the working slot the action names.
...
detection on flex_1, elution Buffer B and Buffer D on flex_2. A PLR Trough is one
...
racks would not survive a full plate). All racks are 200 uL: the Flex p1000 aspirates
every SMC volume (10-100 uL) from a 200 uL tip, and the PLR fork exposes no 50 uL or
Flex-named tip def. Sample-to-sample transfers change tips every column; a single
reagent drawn from a trough reuses one 8-tip set across the plate.

All reagent labware stays stationary on the Flex decks across executions via
``REUSE_EXISTING`` + ``LEAVE_IN_PLACE``.

Buffer D pre-load and the eluate transfer both land in the neutralization plate at
flex_2, so they are one action driven by the assay plate (which already holds
flex_2 from eluting); the fresh neutralization plate stays a passive receiver
that never reserves the device, which a second driving thread would deadlock
against.

Submission shape (same as the Hamilton SMC example):
...
# Deck-resident reagent troughs (replenished: seeded full, non-depleting in sim).
# hamilton_1_trough_200ml_Vb: the one wired single-pool trough that defines
# material_z_thickness, which the Flex robot-server's aspirate (cavity_bottom) requires.
...
# One tip rack per pipetting action, all 200 uL (the p1000 covers every SMC volume;
# the PLR fork has no 50 uL tip def). A fresh-tips-per-column transfer uses a whole rack.
tips_sample = TipRackTemplate("tips_sample", labware_type="opentrons_96_filtertiprack_200ul", with_tips=True)
tips_beads = TipRackTemplate("tips_beads", labware_type="opentrons_96_filtertiprack_200ul", with_tips=True)
tips_detection = TipRackTemplate("tips_detection", labware_type="opentrons_96_filtertiprack_200ul", with_tips=True)
tips_read = TipRackTemplate("tips_read", labware_type="opentrons_96_filtertiprack_200ul", with_tips=True)
tips_buffer_b = TipRackTemplate("tips_buffer_b", labware_type="opentrons_96_filtertiprack_200ul", with_tips=True)
tips_buffer_d = TipRackTemplate("tips_buffer_d", labware_type="opentrons_96_filtertiprack_200ul", with_tips=True)
tips_eluate = TipRackTemplate("tips_eluate", labware_type="opentrons_96_filtertiprack_200ul", with_tips=True)
...
def build_workflow(topology: Topology) -> WorkflowTemplate:
    """Build the Opentrons Flex SMC (IL-6) assay workflow bound to the given topology."""

    biotek_1 = topology.device("biotek_1", PlateWasher)
    biotek_2 = topology.device("biotek_2", PlateWasher)
    flex_1 = topology.device("flex_1", LiquidHandler)
    flex_2 = topology.device("flex_2", LiquidHandler)
    ...
    @orca.action(
        device=flex_1, inputs=[detection_reservoir, plate_1, tips_detection],
        deck_positions={
            plate_1: "C2-slot",
            # Own slot (not A1, which tips_sample uses in target_capture): in a
            # batch, one submission's tips_sample can still hold A1 when another's
            # tips_detection needs it, blocking convergence.
            tips_detection: "B3-slot",
        },
    )
    ...
    @orca.action(
        device=flex_2, inputs=[buffer_b_reservoir, plate_1, tips_buffer_b],
        deck_positions={
            plate_1: "C1-slot",
            tips_buffer_b: "A1-slot",
        },
    )
    ...
    # Pellet the beads on the magnet for 2 min before transferring the eluate, so the
    # supernatant transfers bead-free. With no standalone magnet device, the
    # biotek washer's magnet holds the plate.
    @orca.action(device=biotek_2, inputs=[plate_1])
    async def run_magnetic_pellet(ctx: ActionContext):
        await ctx.device().run_protocol("magnetic_pellet.pro", {})

    # Pre-dispense 10 uL Buffer D into the fresh neutralization plate, then transfer
    # 10 uL eluate from the assay plate ONTO it (neutralizing). Driven by the assay
    # plate (already on flex_2 from eluting); the neutralization plate joins as a
    # passive destination. Buffer D reuses tips; the eluate changes tips per column.
    @orca.action(
        device=flex_2,
        ...
        deck_positions={
            plate_1: "C1-slot",
            neut_plate: "C2-slot",
            # Each flex_2 tip rack gets its own slot so batched submissions never
            # block each other on a shared slot: A1 tips_buffer_b, A2 tips_eluate,
            # B1 tips_buffer_d, B2 tips_read (D1/D2 are the two handoff lanes).
            tips_buffer_d: "B1-slot",
            tips_eluate: "A2-slot",
        },
    )
    ...
    # Transfer the neutralized eluate into the 384 read plate (20 uL = 10 Buffer D +
    # 10 eluate). The neutralization plate drives; final_plate joins. Each contributor
    # writes its own interleaved quadrant, keyed by ctx.pool_index.
    @orca.action(
        device=flex_2, inputs=[neut_plate, final_plate, tips_read],
        deck_positions={
            neut_plate: "C2-slot",
            # The 384 read plate is a long-lived receiver: it camps on the Flex
            # across every contribution, so it gets its own slot (C3) rather than
            # plate_1's C1, which the assay plates need for elution.
            final_plate: "C3-slot",
            tips_read: "B2-slot",
        },
    )
    ...
    # magazine). Every rack is a 200 uL filter tip; the p1000 covers every SMC volume.
    ...
    # Deck-resident reagent threads: each trough stays on its host Flex's reagent
    # slot. REUSE_EXISTING rebinds the persistent labware each run; LEAVE_IN_PLACE
    # keeps it on the deck. The has_more_work loop re-offers the resident once per
    # consuming sample plate (an N-group batch reuses it N times).
    @orca.thread(
        labware=bead_reservoir,
        start=("flex_1/B1-slot", REUSE_EXISTING),
        end=("flex_1/B1-slot", LEAVE_IN_PLACE),
    )
    ...
    @orca.thread(
        ...
        end=("flex_1/B2-slot", LEAVE_IN_PLACE),
    )
    ...
    @orca.thread(
        ...
        start=("flex_2/B3-slot", REUSE_EXISTING),
        end=("flex_2/B3-slot", LEAVE_IN_PLACE),
    )
    ...
    @orca.thread(
        ...
        start=("flex_2/D3-slot", REUSE_EXISTING),
        end=("flex_2/D3-slot", LEAVE_IN_PLACE),
    )
    ...
    @orca.workflow(name="opentrons_flex_smc_assay")
