"""Physical topology for the Opentrons Flex SMC assay example.

The SMC bench that the Hamilton example (``examples/hamilton_smc``) models with
two ML STARs, re-instrumented with two Opentrons Flex liquid handlers:

  * ``mlstar_1`` -> ``flex_1``  (DDR-1 zone, capture + detection)
  * ``mlstar_2`` -> ``flex_2``  (DDR-3 zone, elution + neutralize + read)

The SMC assay routes a fresh tip box around the system map per pipetting action
(stacker -> deck -> waste). On a Hamilton that is physically impossible: tips are
racks that clip into human-loaded tip carriers, so a transporter arm cannot
deliver one to a deck site. On an Opentrons a tip box sits on a flat deck slot
exactly like a plate, so the arm can pick and place it. The Flex (not the OT-2)
is the target because relaying a dropped box from the handoff slot to the working
slot needs the liquid handler's own internal gripper, which the Flex has and the
OT-2 does not.

The shared devices (3 DDR zones, 2 translator bridges, 8 stackers, 10 shakers,
9 parking pads, biotek washers, centrifuge, sealer, delidder, reader, plate
hotel, waste) match the Hamilton SMC topology. This bench additionally carries 4
thermocyclers (available for a TruSeq library-prep workflow; the SMC assay does
not use them).
...
binds a small ``_SmcDeckFactory`` so both Flexes fall back to
...
Flexes dispatch to orca-client over the WebSocket. Tests that need to inspect
the pushed calls bind their own factory first (one returning a
``RecordingLiquidHandlerDriver`` per Flex); the outer binding wins.
"""
...
# Two handoff lanes per Flex: the DDR arm is taught a plate-handoff slot (D1) and
# a tip-handoff slot (D2); it drops transit labware there and the Flex's internal
# gripper relays it to the deck_positions= working slot named by the action. Two
# lanes (mirroring Hamilton's two handoff carriers) keep plate and tip traffic
# from serializing on one slot in a batch. Troughs are deck residents anchored via
# @orca.thread(start=("<flex>/<slot>-slot", REUSE_EXISTING)); transit labware
# enters via @orca.action(deck_positions=...). A Flex deck's slots (A1..D3, A3 is
# the trash) are built-in structure, so DeckLayoutConfig declares no resources --
# the child Locations (e.g. flex_1/C2-slot) are derived from the deck's slot list.
FLEX_1_DECK = DeckLayoutConfig(deck_type="FlexDeck", resources=[])
FLEX_2_DECK = DeckLayoutConfig(deck_type="FlexDeck", resources=[])


class _SmcDeckFactory:
    """Default factory for this example: a fresh ChatterboxLH per Flex, sim
    ...
    in its own ``use_device_factory(...)`` (e.g. to inject a per-Flex
...
def build_topology(stores: IRuntimeStoreFactory) -> Topology:
    """Build a fresh Opentrons Flex SMC assay topology (Hamilton SMC bench with
    the two ML STARs replaced by two Flexes).

    Layout: 3 DDR robot zones connected by 2 translator bridges.

      DDR-1 (zone 1): biotek_1, flex_1, waste_1, pads 1-3
        |
      translator_1
        |
      ...
      translator_2
        |
      DDR-3 (zone 3): biotek_2, flex_2, plate_hotel, smc_pro, waste_2, pads 7-9
    ...
    with ctx:
        ...
        ddr_1_teachpoints = [
            ...
            Teachpoint("flex_1/D1-slot",      c(300, 200, 300, 0, 90, 180), orientation="right", access=default_vertical),
            Teachpoint("flex_1/D2-slot",      c(320, 200, 300, 0, 90, 180), orientation="right", access=default_vertical),
        ...
        ddr_3_teachpoints = [
            ...
            Teachpoint("flex_2/D1-slot",      c(2100, 200, 300, 0, 90, 180), orientation="right", access=default_vertical),
            Teachpoint("flex_2/D2-slot",      c(2120, 200, 300, 0, 90, 180), orientation="right", access=default_vertical),
        ...
        flex_2 = LiquidHandler(
            "flex_2",
            deck_layout_store=stores.deck_layouts("flex_2", seed={"default": FLEX_2_DECK}),
            deck_layout="default",
        )
