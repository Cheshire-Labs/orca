"""Deployment-package shape for the Opentrons Flex SMC assay.
...
as ``examples.opentrons_flex_smc`` (orca-core tests / direct runs) or renamed
...
The workflow lives at ``workflows/opentrons_flex_smc_assay.py`` like any other
...
them onto the Flex decks and the ``REUSE_EXISTING`` resident threads bind to the
...
async def build(stores: IRuntimeStoreFactory) -> SystemBuild:
    """Construct the Opentrons Flex SMC system + auto-register its workflows."""
    ...
    system_build = await orca.build_system(
        name="Opentrons Flex SMC Assay",
        topology=topology,
        stores=stores,
        labwares=reservoirs,
        description="SMC immunoassay authoring real inline liquid-handler methods "
                    "(aspirate/dispense/tips) pushed via PyLabRobot on an Opentrons "
                    "Flex pair (flex_1 capture/detection, flex_2 elution/neutralize)",
    )
